    ...
    def checkpoint(name, checkpoint, time_from_start=False, print_directly=False):
        ...
        try:
            if name in TimeTracker.times:
                ...
                begin_time = (
                    TimeTracker.times[name]["start"]
                    if time_from_start
                    else TimeTracker.times[name]["last checkpoint"]
                )
                execution_time = round(current_time - begin_time, 2)
                TimeTracker.times[name]["last checkpoint"] = current_time
                if print_directly:
                    LogPlus.timer(
                        f"| TimeTracker | {name} took {execution_time} ms for {checkpoint}"
                    )
                # add it to recorded checkpoints
                if checkpoint in TimeTracker.times[name]["checkpoints"]:
                    TimeTracker.times[name]["checkpoints"][checkpoint].append(
                        execution_time
                    )
                else:
                    TimeTracker.times[name]["checkpoints"][checkpoint] = [
                        execution_time
                    ]
            else:
                LogPlus.error(
                    f"| TimeTracker | Couldn't print checkpoint for {name}, it wasn't started"
                )
        except Exception as e:
        ...
        try:
            if name in TimeTracker.times:
                ...
                execution_time = round(
                    current_time - TimeTracker.times[name]["start"], 2
                )
                TimeTracker.times[name]["ends"].append(execution_time)
                if print_directly:
                    LogPlus.timer(
                        f"| TimeTracker | {name} took {execution_time} ms in total"
                    )
            else:
                LogPlus.error(
                    f"| TimeTracker | Couldn't print total time for {name}, it wasn't started"
                )
        except Exception as e:
            ...
            if name in TimeTracker.times:
                ...
                for checkpoint in TimeTracker.times[name]["checkpoints"]:
                    if len(TimeTracker.times[name]["checkpoints"][checkpoint]) > 0:
                        exec_time = round(
                            sum(TimeTracker.times[name]["checkpoints"][checkpoint])
                            / len(TimeTracker.times[name]["checkpoints"][checkpoint]),
                            2,
                        )
                    else:
                ...
                exec_time = round(
                    sum(TimeTracker.times[name]["ends"])
                    / len(TimeTracker.times[name]["ends"]),
                    2,
                )
                LogPlus.timer(f"| TimeTracker | Total: {exec_time} ms")
            else:
                LogPlus.error(
                    f"| TimeTracker | Couldn't print stats for {name}, it wasn't started"
                )
        except Exception as e:
