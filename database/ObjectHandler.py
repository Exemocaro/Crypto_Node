...
class ObjectHandler:
    ...
    @staticmethod
    def add_object(
        obj: typing.Dict, validity="valid", object_type="unknown", sender_address=None
    ):
    ...
    def get_objects_with_status(status: str):
        ...
        return [
            obj[object_key]
            for obj in ObjectHandler.objects
            if obj[validity_key] == status
        ]
    ...
    def update_pending_objects(object_id):
        ...
        try:
            ...
            pending_objects = [
                obj for obj in ObjectHandler.objects if obj[validity_key] == "pending"
            ]
        ...
        try:
            ...
            for obj in pending_objects:
                ...
                if not pending_key in obj or obj[pending_key] == None:
                    obj[pending_key] = []
            pending_objects = [
                obj for obj in pending_objects if len(obj[missing_key]) == 0
            ]
            pending_objects = [
                obj for obj in pending_objects if len(obj[pending_key]) == 0
            ]
    ...
    def get_block_containing_object(object_id):
        ...
        blocks = [
            obj[object_key] for obj in ObjectHandler.objects if obj[type_key] == "block"
        ]
        for block in blocks:
    ...
    def get_chaintip():
        ...
        blocks = [
            obj[txid_key]
            for obj in ObjectHandler.objects
            if obj[type_key] == "block" and obj[validity_key] == "valid"
        ]
        if len(blocks) <= 1:
