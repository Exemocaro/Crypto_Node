            ...
            for new_txid in txs[1:]:
                ...
                for input in tx[inputs_key]:
                    ...
                    if txid_to_check in new_set.keys():
                        ...
                        LogPlus.error(f"| ERROR | UTXO | Last | Couldn't find txid {txid_to_check} in set {new_set}")
                        LogPlus.debug(f"| DEBUG | UTXO | Last | Prev Set: {prev_set}")
                        LogPlus.debug(f"| DEBUG | UTXO | Last | Prev id: {prev_id}")
