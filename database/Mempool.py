...
class Mempool:
    ...
    def add_transaction(transaction: typing.Dict) -> bool:
        ...
        try:
            ...
            LogPlus.warning(f"| WARNING | Mempool | Invalid transaction | {Object.get_id_from_json(transaction)[:10]}...")
            return False
        except Exception as e:
            LogPlus.error(f"| ERROR | Mempool | Couldn't add transaction to mempool | {e} | {Object.get_id_from_json(transaction)[:10]}...")
            return False
    ...
    def apply_block(block: typing.Dict):
        ...
        for tx_id in Mempool.mempool_txids:
            ...
            if not Mempool.is_transaction_valid_for_mempool(tx):
                ...
                Mempool.mempool_utxo = UTXO.apply_transaction_to_UTXO(Mempool.mempool_utxo, tx)
